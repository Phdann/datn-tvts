...
const SUGGESTED_TOPICS = [
    ...
    "Học phí & Học bổng 2026",
    "Phương thức xét tuyển mới nhất",
    "Cơ hội việc làm sau tốt nghiệp",
    "Đời sống sinh viên tại DUE"
];
...
- **XỬ LÝ KHI CÂU HỎI KHÔNG RÕ RÀNG (QUAN TRỌNG):**
  - Nếu người dùng hỏi chung chung (vd: "điểm chuẩn", "học phí", "tuyển sinh") mà không nói rõ ngành hay năm, bạn PHẢI hỏi lại để làm rõ ý của họ.
  - Ví dụ: "Bạn đang quan tâm đến điểm chuẩn của ngành nào trong năm 2026 vậy?" hoặc "Mình có thể cung cấp thông tin học phí cho từng ngành, bạn muốn biết ngành nào?".
  - Luôn đi kèm với các gợi ý cụ thể để người dùng dễ dàng chọn lựa.
  - Tuyệt đối không tự bịa thông tin nếu Context không có.
`,
            parent: `
${baseInstruction}
...
**YÊU CẦU VỀ SỰ RÕ RÀNG:**
Nếu câu hỏi của người dùng mơ hồ hoặc thiếu thông tin (ví dụ chỉ hỏi "điểm chuẩn"), bạn hãy đặt một câu hỏi ngược lại thật thân thiện để làm rõ nhu cầu của họ. 
Đồng thời, hãy gợi ý cho họ một vài lựa chọn phổ biến.
`;
                    ...
                    // Phát hiện nếu câu hỏi mơ hồ hoặc AI đang hỏi lại
                    const isAmbiguous = message.length < 15 || reply.includes('?') || !context.text || context.text.length < 300;
                    
                    if (isAmbiguous) {
                        // Trộn và lấy 3 gợi ý liên quan đến năm 2026
                        response.suggestions = SUGGESTED_TOPICS
