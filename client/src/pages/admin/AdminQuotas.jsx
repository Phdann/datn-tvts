...
import { Plus, Pencil, Trash2, Database, Save, X, Search, Target, Eye, EyeOff, RefreshCw, Upload, Globe, Image as ImageIcon } from 'lucide-react';
...
/* ─── Inline row editor ─── */
function QuotaRow({ method, index, isSelected, onSelect, onSave, onConfirmDelete }) {
      ...
      <tr className="bg-blue-50/50">
        ...
        <td className="px-4 py-2">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
             <button 
               type="button"
               onClick={() => document.getElementById(`edit-quota-image-${method.id}`).click()}
             ...
             <div className="flex gap-2 flex-wrap pt-1">
               {form.image_urls.map((url, i) => (
                 <div key={`kept-${i}`} className="relative group/img animate-in zoom-in-50">
                   ...
                   <button type="button" onClick={() => setForm(f => ({...f, image_urls: f.image_urls.filter((_, idx) => idx !== i)}))} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-1 shadow-md opacity-0 group-hover/img:opacity-100 transition-all scale-75 hover:scale-100"><X className="w-2.5 h-2.5"/></button>
                 </div>
               ))}
               {imageFiles.map((file, i) => (
                 <div key={`new-${i}`} className="relative group/img animate-in zoom-in-50">
                   ...
                   <button type="button" onClick={() => setImageFiles(f => f.filter((_, idx) => idx !== i))} className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-1 shadow-md opacity-0 group-hover/img:opacity-100 transition-all scale-75 hover:scale-100"><X className="w-2.5 h-2.5"/></button>
                 </div>
               ))}
...
function AddForm({ onSave, onCancel }) {
  const [form, setForm] = useState({ name: '', image_urls: [], year: new Date().getFullYear(), is_public: true, type: 'quota' });
  const [imageFiles, setImageFiles] = useState([]);
  const [imageUrlInput, setImageUrlInput] = useState('');
  const [saving, setSaving] = useState(false);
  const toast = useToast();
  ...
  const save = async (e) => {
    ...
    try {
      ...
      toast.error('Lỗi khi thêm chỉ tiêu');
    }
    setSaving(false);
  };
    ...
    <tr className="bg-emerald-50/60 border-t-2 border-emerald-300">
      ...
      <td className="px-4 py-2">
        <input
          ...
          placeholder="Tiêu đề thông báo chỉ tiêu *"
      ...
      <td className="px-4 py-2">
        <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm space-y-3">
           <button 
             type="button"
             onClick={() => document.getElementById('add-quota-image').click()}
...
export default function AdminQuotas() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setSelectedIds([]);
      const d = await admissionMethodService.getAllAdmissionMethods({ limit: 100, type: 'quota' });
  ...
  const handleCreate = async (formData) => {
    try {
      ...
      toast.error(e.response?.data?.message || 'Lỗi tạo chỉ tiêu');
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-blue-950 tracking-tight uppercase flex items-center gap-3">
              <div className="p-2 bg-blue-50 rounded-xl"><Database className="w-6 h-6 text-blue-600" /></div>
              Quản lý Chỉ tiêu
            </h1>
            <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mt-1">Thiết lập chỉ tiêu tuyển sinh hàng năm</p>
          </div>
        </div>
        <button
            ...
            <table className="w-full text-sm text-left border-separate border-spacing-0">
              <thead className="bg-slate-50/50">
                <tr className="text-[10px] text-slate-400 font-black uppercase tracking-widest">
                  ...
                  <th className="px-4 py-5 border-b border-slate-100 whitespace-nowrap">Ảnh văn bản/sơ đồ</th>
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-32 text-center">
                       <div className="flex flex-col items-center gap-3 opacity-20">
                          ...
                          <p className="font-black text-sm uppercase tracking-widest">{search ? 'Không tìm thấy kết quả nào phù hợp' : 'Chưa có thông báo chỉ tiêu nào'}</p>
                       </div>
                    </td>
                  </tr>
                )}
