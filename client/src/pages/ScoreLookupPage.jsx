...
import {
  Search, TrendingUp, ChevronRight, ChevronDown, ArrowRight,
  Target, Calendar, Award, GraduationCap,
} from 'lucide-react';
    ...
    <section className="relative bg-slate-900 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-14 lg:py-20">
        <div className="flex items-center gap-2 text-xs text-white/50 mb-8">
          ...
          <span className="text-white/80">Tra cứu điểm chuẩn</span>
        </div>
        <div className="max-w-3xl">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight tracking-tight mb-5">
            Điểm chuẩn
            <br />
            <span className="text-primary-light">Các năm</span>
          </h1>
          <p className="text-sm sm:text-base text-white/70 leading-relaxed max-w-2xl">
            Tra cứu các thông báo điểm chuẩn trúng tuyển chính thức của Trường Đại học Kinh tế — Đại học Đà Nẵng qua từng năm.
          </p>
        </div>
      </div>
    </section>
...
/* ──────────────── POST CARD ──────────────── */
function BenchmarkPostCard({ post }) {
  return (
      ...
      <div className="p-6 md:p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className="text-xs font-bold text-violet-600 bg-violet-50 px-3 py-1 rounded-lg uppercase tracking-wider">
              Điểm chuẩn
            </span>
            {post.year && (
              <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1 rounded-lg border border-slate-200">
                Tuyển sinh {post.year}
              </span>
            )}
          </div>
          ...
          if (allImages.length === 0) {
            return (
              <div className="py-20 flex flex-col items-center justify-center text-slate-300 w-full">
                ...
                <p className="text-sm font-medium">Bảng điểm đang được cập nhật...</p>
              </div>
            );
          }
...
export default function ScoreLookupPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setLoading(true);
        const res = await api.get('/admission-methods', { params: { type: 'benchmark', limit: 100 } });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <p className="text-[11px] text-slate-500 font-medium">Chọn năm tuyển sinh bạn muốn tra cứu</p>
             </div>
          </div>
          <div className="relative w-full sm:w-48">
            <select
              ...
              className="w-full pl-4 pr-10 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 focus:outline-none focus:border-primary appearance-none cursor-pointer font-bold text-slate-700 shadow-none hover:bg-slate-100 transition-colors"
            >
        ...
        {filteredPosts.length === 0 ? (
          <div className="bg-white border border-dashed border-slate-200 py-24 rounded-3xl text-center shadow-inner">
            ...
            <p className="text-base text-slate-500 font-bold uppercase tracking-wider">Chưa có thông báo điểm chuẩn</p>
            <p className="text-sm text-slate-400 mt-2">Dữ liệu sẽ được Ban tuyển sinh cập nhật sớm nhất</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredPosts.map(post => (
              <BenchmarkPostCard key={post.id} post={post} />
            ))}
          </div>
        )}
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-16 flex flex-col lg:flex-row items-center justify-between gap-10">
          <div>
            <h2 className="text-3xl font-black text-white leading-tight">
              Đánh giá <span className="text-primary-light">Cơ hội Trúng tuyển?</span>
            </h2>
            <p className="mt-4 text-white/60 max-w-lg text-sm font-medium">
              Sử dụng công cụ tính điểm xét tuyển để xem bạn có phù hợp với điểm chuẩn của các năm trước hay không.
            </p>
          </div>
          <Link to="/tinh-diem-xet-tuyen" className="inline-flex items-center gap-3 bg-primary text-white font-black px-10 py-4 rounded-xl hover:bg-primary-light transition-all shadow-xl shadow-primary/25 border border-primary/20">
            Tính điểm ngay <ArrowRight className="w-5 h-5" />
          </Link>
        </div>
      </section>
    </div>
